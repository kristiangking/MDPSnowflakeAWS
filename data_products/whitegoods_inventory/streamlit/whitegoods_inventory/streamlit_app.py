...
locations = session.sql(
    "SELECT DISTINCT location_name FROM WHITEGOODS_ANALYTICS.marts.mart_inventory_summary ORDER BY 1"
).to_pandas()["LOCATION_NAME"].tolist()

categories = session.sql(
    "SELECT DISTINCT category FROM WHITEGOODS_ANALYTICS.marts.mart_inventory_summary ORDER BY 1"
...
with tab1:
    ...
    inv = session.sql(f"""
        ...
        FROM WHITEGOODS_ANALYTICS.marts.mart_inventory_summary
